...
contract AgentCallManagerWithVerify {
    ...
    function invokeAgent(uint256 agentId) external payable returns (bytes32) {
        // destructure needed fields from the public getter tuple
        uint256 fee;
        bool active;
        (, , , , fee, active, ) = registry.agents(agentId);
    ...
    function confirmResult(
        ...
        (, agentOwner, , , , , expectedSigner) = registry.agents(agentId);
        require(expectedSigner != address(0), "agent signer not set");
        ...
        address payable recipient = payable(agentOwner);
